...
function runAnalysis() {
  ...
  // The lead is whoever actually scores highest - not a hard-coded answer.
  const topId = SUSPECTS.reduce((best, s) => (scores[s.id].sim > scores[best].sim ? s.id : best), SUSPECTS[0].id);

  // Render the real cosine similarity for each suspect (guarded against an all-zero note).
  SUSPECTS.forEach((s) => {
    const { sim, feats, shared } = scores[s.id];
    const pct = max > 0 ? Math.round(sim * 100) : 0;
